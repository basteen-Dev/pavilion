...
export async function getProducts(searchParams) {
    ...
    `;
        ...
        if (sub_category) {
            queryStr += ` AND p.sub_category_id = $${paramCount++}`;
            queryParams.push(sub_category);
        }
        if (brand) {
            queryStr += ` AND p.brand_id = $${paramCount++}`;
            queryParams.push(brand);
        }
        if (collection_id) {
            queryStr += ` AND c.parent_collection_id = $${paramCount++}`;
            queryParams.push(collection_id);
        }
        if (price_min) {
            queryStr += ` AND p.selling_price >= $${paramCount++}`;
        ...
        if (is_featured) {
            queryStr += ` AND p.is_featured = $${paramCount++}`;
        ...
        // Generate count query dynamically based on the final queryStr (excluding SELECT * ...)
        // We need to re-construct or handle the count query carefully to include all joins and where clauses.
        // A safer way is to replace 'SELECT p.* ...' with 'SELECT COUNT(*)' but keep joins as collection filter depends on join.
        const countQueryStr = `SELECT COUNT(*) FROM products p 
                               LEFT JOIN brands b ON p.brand_id = b.id
                               LEFT JOIN categories c ON p.category_id = c.id
                               LEFT JOIN sub_categories sc ON p.sub_category_id = sc.id
                               ${queryStr.split('WHERE p.is_active = true')[1] ? 'WHERE p.is_active = true' + queryStr.split('WHERE p.is_active = true')[1] : 'WHERE p.is_active = true'}`;
        ...
        const [productsResult, countResult] = await Promise.all([
            ...
            query(countQueryStr, queryParams.slice(0, queryParams.length - 2))
        ]);
